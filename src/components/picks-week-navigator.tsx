...
interface PicksWeekNavigatorProps {
  currentWeek: number;
  currentSeasonType: number;
}

export function PicksWeekNavigator({
  currentWeek,
  currentSeasonType,
}: PicksWeekNavigatorProps) {
  ...
  const navigateWeek = (direction: "prev" | "next") => {
    ...
    router.push(`/picks/${newSeasonType}/${newWeek}`);
  };

  const navigateSeasonType = (seasonType: number) => {
    router.push(`/picks/${seasonType}/1`);
  };
  ...
  return (
    <div className="mb-6">
      ...
      <h1 className="lg:hidden text-2xl font-bold text-base-content mb-4">Make Your Picks</h1>
